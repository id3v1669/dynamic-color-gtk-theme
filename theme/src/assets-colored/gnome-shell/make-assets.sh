...
for theme in '' '-Purple' '-Pink' '-Red' '-Orange' '-Yellow' '-Green' '-Blue' '-Grey'; do
    for type in '' '-Storm' '-Moon'; do
        ...
        if [[ "$type" != '' ]]; then
            ...
            sed -i "s/#fe8019/${theme_color_dark}/g" "theme${theme}${type}"/*.svg
            sed -i "s/#af3a03/${theme_color_light}/g" "theme${theme}${type}"/*.svg
        elif [[ "$theme" != '' ]]; then
            rm -rf "theme${theme}"
            cp -rf "theme" "theme${theme}"
            sed -i "s/#fe8019/${theme_color_dark}/g" "theme${theme}"/*.svg
            sed -i "s/#af3a03/${theme_color_light}/g" "theme${theme}"/*.svg
        fi
    done
done
